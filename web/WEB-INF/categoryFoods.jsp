...
<%
    ...
    ArrayList<FoodDTO> categoryFoods = dao.getAllFoods();

    if(category != null) {
        categoryFoods = dao.findFoodsByCategory(category);
    }
%>
...
<div class="container-fluid bg-secondary mb-5">
    <div class="d-flex flex-column align-items-center justify-content-center" style="min-height: 300px">
        <%
            if(category == null) {
        %>
        <h1 class="font-weight-semi-bold text-uppercase mb-3">전체 안주 전체보기</h1>
        <div class="d-inline-flex">
            <p class="m-0"><a href="home.do">Home</a></p>
            <p class="m-0 px-2">-</p>
            <p class="m-0">전체 안주 전체보기</p>
        </div>
        <%} else {%>
        <h1 class="font-weight-semi-bold text-uppercase mb-3"><%=category%> 안주 전체보기</h1>
        <div class="d-inline-flex">
            <p class="m-0"><a href="home.do">Home</a></p>
            <p class="m-0 px-2">-</p>
            <p class="m-0"><%=category%> 안주 전체보기</p>
        </div>
        <%}%>
    </div>
</div>
    ...
    <div class="row px-xl-5 pb-3">
        <%
            for(int i=0; i< categoryFoods.size(); i++) {
                FoodDTO food = categoryFoods.get(i);
        %>
